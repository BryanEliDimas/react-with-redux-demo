import React, { Component } from 'react'
import { connect } from 'react-redux'

class WithStateProps extends Component {
  constructor(props) {
    super(props);
  }

  flipBool = () => {
    this.props.dispatch({ type: 'FLIP_BOOL_ACTION' })
  }

  render() {
    return(
      <div>
        <h3>WithStateProps.js</h3>
        <div>
          A piece of the state will get added to this component as a prop!
          And it feels like magic :). Value of boolean action:&nbsp;
          { String(this.props.stateAsProp) }
        </div>

        <div>Also, <code>dispatch</code> will get added as a prop</div>
        <button onClick={this.flipBool}>Flip</button>
      </div>
    )
  }
}

const mapStateToProps = (state) => ({
  stateAsProp: state.boolAction
})

export default connect(mapStateToProps)(WithStateProps)
